...
namespace IocPerformance.Adapters
{
	public sealed class SimpleInjectorContainerAdapter : ContainerAdapterBase
	{
		...
		private void RegisterPropertyInjection()
		{
			...
			this.container.RegisterInitializer<SubObjectA>(x => x.ServiceA = container.GetInstance<IServiceA>());

			this.container.Register<ISubObjectB, SubObjectB>();
			this.container.RegisterInitializer<SubObjectB>(x => x.ServiceB = container.GetInstance<IServiceB>());

			this.container.Register<ISubObjectC, SubObjectC>();
			this.container.RegisterInitializer<SubObjectC>(x => x.ServiceC = container.GetInstance<IServiceC>());
			...
			this.container.RegisterInitializer<ComplexPropertyObject>(x =>
				                                                          {
					                                                          x.ServiceA = container.GetInstance<IServiceA>();
					                                                          x.ServiceB = container.GetInstance<IServiceB>();
					                                                          x.ServiceC = container.GetInstance<IServiceC>();
					                                                          x.SubObjectA = container.GetInstance<ISubObjectA>();
					                                                          x.SubObjectB = container.GetInstance<ISubObjectB>();
					                                                          x.SubObjectC = container.GetInstance<ISubObjectC>();
				                                                          });
		}
